...
class GeneralIORayPPOTrainer(ReasonRLRayPPOTrainer):
    ...
    def _compute_batch(self, batch: DataProto, metrics: dict, timing_raw: dict, problem_type: str ) -> tuple[DataProto, dict]:
        ...
        # === EVOLVE PROMPT DEBUG: Input Prompt Templates ===
        # if hasattr(self, 'prompt_manager') and self.prompt_manager is not None:
        #     current_prompts = self.prompt_manager.get_current_prompts()
        #     PrettyPrinter.section_header(f"[EVOLVE PROMPT DEBUG] Current Prompt Templates for {problem_type}")
        #     for prompt_type, prompt_content in current_prompts.items():
        #         if prompt_type in ['generation_prompt', 'prediction_prompt', 'judge_prompt']:
        #             PrettyPrinter.status(f"TEMPLATE TYPE", prompt_type, "info")
        #             PrettyPrinter.status(f"TEMPLATE CONTENT", prompt_content[:300] + "..." if len(prompt_content) > 300 else prompt_content, "info")
        #             print("-" * 60)
        
        # === EVOLVE PROMPT DEBUG: Show Input Data Before Generation ===
        # if hasattr(batch, 'batch') and 'input_ids' in batch.batch:
        #     input_texts = [self.tokenizer.decode(ids, skip_special_tokens=True) for ids in batch.batch['input_ids'][:2]]
        #     PrettyPrinter.section_header(f"[EVOLVE PROMPT DEBUG] Input Data for {problem_type}")
        #     for i, input_text in enumerate(input_texts):
        #         PrettyPrinter.status(f"INPUT PROMPT {i+1}", input_text[:400] + "..." if len(input_text) > 400 else input_text, "info")
        #         print("-" * 40)
        ...
        # === EVOLVE PROMPT DEBUG: Actor Generated Outputs ===
        # if gen_batch_output.batch.get('responses') is not None:
        #     output_ids = gen_batch_output.batch['responses']
        #     output_texts = [self.tokenizer.decode(ids, skip_special_tokens=True) for ids in output_ids[:3]]
        #     PrettyPrinter.section_header(f"[EVOLVE PROMPT DEBUG] Actor Generated Outputs for {problem_type}")
        #     for i, output_text in enumerate(output_texts):
        #         PrettyPrinter.status(f"GENERATED OUTPUT {i+1}", output_text[:500] + "..." if len(output_text) > 500 else output_text, "success")
        #         print("-" * 60)
                        ...
                        with _timer('testing', timing_raw):
                            ...
                            if val_metrics:
                                ...
                                if hasattr(self, 'prompt_manager') and hasattr(self, 'benchmark_tracker'):
                                    try:
                                        # === EVOLVE PROMPT DEBUG: Benchmark Analysis ===
                                        PrettyPrinter.section_header("[EVOLVE PROMPT DEBUG] Starting Prompt Evolution Analysis")
                                        ...
                                        PrettyPrinter.status("IMPROVEMENT PROMPTS", f"Generated {len(improvement_prompts) if improvement_prompts else 0} improvement suggestions", "info")
                                        
                                        if improvement_prompts:
                                            PrettyPrinter.section_header("[EVOLVE PROMPT DEBUG] Raw Improvement Suggestions")
                                            for i, prompt_suggestion in enumerate(improvement_prompts[:3]):  # Show first 3
                                                PrettyPrinter.status(f"SUGGESTION {i+1}", prompt_suggestion[:300] + "..." if len(prompt_suggestion) > 300 else prompt_suggestion, "info")
                                            print("-" * 80)
                                        ...
                                        if improvement_prompts:
                                            ...
                                            # === EVOLVE PROMPT DEBUG: Performance Context ===
                                            PrettyPrinter.section_header("[EVOLVE PROMPT DEBUG] Performance Analysis")
                                            PrettyPrinter.status("PERFORMANCE CONTEXT", performance_context, "info")
                                            if analysis_report:
                                                PrettyPrinter.status("FULL ANALYSIS", analysis_report[:500] + "..." if len(analysis_report) > 500 else analysis_report, "info")
                                            print("-" * 80)
                                            
                                            # Use actor-driven optimization if enabled
                                            if self.enable_actor_optimization and self.actor_prompt_optimizer:
                                                PrettyPrinter.section_header("[EVOLVE PROMPT DEBUG] Actor-Driven Optimization Starting")
                                                ...
                                                # === EVOLVE PROMPT DEBUG: Input Analysis Data ===
                                                PrettyPrinter.status("PROBLEMATIC QUESTIONS", f"Found {len(problematic_questions) if problematic_questions else 0} problematic questions", "info")
                                                if problematic_questions:
                                                    for i, question in enumerate(problematic_questions[:2]):  # Show first 2
                                                        PrettyPrinter.status(f"PROBLEM {i+1}", question + "..." if len(question) > 200 else question, "warn")
                                                
                                                PrettyPrinter.status("PERFORMANCE TRENDS", f"Trends analysis: {str(performance_trends)}..." if len(str(performance_trends)) > 300 else str(performance_trends), "info")
                                                print("-" * 60)
                                                
                                                # === EVOLVE PROMPT DEBUG: Actor Input Preparation ===
                                                PrettyPrinter.section_header("[EVOLVE PROMPT DEBUG] Preparing Actor Input")
                                                PrettyPrinter.status("BENCHMARK ANALYSIS", f"Analysis length: {len(analysis_report) if analysis_report else 0} chars", "info")
                                                if analysis_report:
                                                    PrettyPrinter.status("ANALYSIS PREVIEW", analysis_report + "..." if len(analysis_report) > 400 else analysis_report, "info")
                                                
                                                # Let actor model optimize prompts
                                                PrettyPrinter.status("ACTOR", "Calling actor model for prompt optimization...", "info")
                                                ...
                                                # === EVOLVE PROMPT DEBUG: Actor Output ===
                                                PrettyPrinter.section_header("[EVOLVE PROMPT DEBUG] Actor Optimization Results")
                                                PrettyPrinter.status("OPTIMIZATION RESULT", f"Generated {len(optimized_prompts) if optimized_prompts else 0} optimized prompts", "success")
                                                
                                                if optimized_prompts:
                                                    for prompt_type, new_prompt in optimized_prompts.items():
                                                        PrettyPrinter.status(f"OPTIMIZED {prompt_type.upper()}", new_prompt + "..." if len(new_prompt) > 400 else new_prompt, "success")
                                                        print("-" * 40)
                                                
                                                # Apply optimized prompts safely
                                                if optimized_prompts:
                                                    PrettyPrinter.section_header("[EVOLVE PROMPT DEBUG] Applying Optimized Prompts")
                                                    
                                                    # Show before and after comparison
                                                    current_prompts = self.prompt_manager.get_current_prompts()
                                                    for prompt_type in optimized_prompts.keys():
                                                        if prompt_type in current_prompts:
                                                            PrettyPrinter.status(f"BEFORE {prompt_type.upper()}", current_prompts[prompt_type] + "..." if len(current_prompts[prompt_type]) > 300 else current_prompts[prompt_type], "warn")
                                                            PrettyPrinter.status(f"AFTER {prompt_type.upper()}", optimized_prompts[prompt_type] + "..." if len(optimized_prompts[prompt_type]) > 300 else optimized_prompts[prompt_type], "success")
                                                            print("-" * 40)
